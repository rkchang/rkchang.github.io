...
Arista Networks
\begin{itemize}
    \item Authored a design document and refactoring plan to resolve critical failure conditions and improve the long-term maintainability of a Syslog forwarding agent."
    \item Patched the agent's C and Python codebase to enable new TCP functionality, specifically for binding to loopback interfaces.
    \item Implemented challenge based authentication for the Radius protocol.
    \item Developing a clang-tidy check that parses C++ and inserts missing tracing calls.
    \item Improved input validation for a high performance C++ logging library.
    \item Worked on customer escalations and debugged issues live with the customer.
    \item Debugged race conditions in network switch authentication and logging tests.
    \item Improved parallelization of tests resulting in a 50\% reduction in runtime
\end{itemize}
